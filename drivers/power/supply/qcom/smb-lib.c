...
static int fih_recover_from_soft_jeita(struct smb_charger *chg)
{
	u8 stat;
	int effective_fv_uv;
	int rc;
	...
	if (chg->jeita_status2 && effective_fv_uv == chg->batt_profile_fv_uv &&
		(stat & BATTERY_CHARGER_STATUS_MASK) == TERMINATE_CHARGE) {
		/*
		 * We are moving from JEITA soft -> Normal and charging
		 * is terminated
		 */
	...
	chg->jeita_status2 = (effective_fv_uv != chg->batt_profile_fv_uv);
...
static void smblib_force_legacy_icl(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
								USB_PSY_VOTER)) {
			if (chg->sdp_current_ua < 0) {
				vote(chg->usb_icl_votable, USB_PSY_VOTER, true, 100000);
			} else {
				smblib_err(chg, "[FIH] Set SDP ICL to %duA\n", chg->sdp_current_ua);
				vote(chg->usb_icl_votable, USB_PSY_VOTER, true, chg->sdp_current_ua);
			}
		}
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, false, 0);
		break;
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP:
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		if (chg->hvdcp_current_ua < 0) {
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 3000000);
		} else {
			smblib_err(chg, "[FIH] Set HVDCP ICL to %duA\n", chg->hvdcp_current_ua);
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, chg->hvdcp_current_ua);
		}
		break;
	default:
...
//		pr_err("input_suspend = %s, usb_online = %s, typec_mode = %s, usb_iSettled = %d, cap = %d, vbat = %d, ibat = %d, FCC(%s) = %d, ICL(override = %s)(%s) = %d, bat_status = %d, bat_temp = %d, resistance = %d, cable_v = %d, cable_i = %d, pmi_temp = %d, parallel_temp = %d, misc_temp_status = %s, system_temp_level = %d\n",
	...
	if (chg->jeita_recover_en)
		fih_recover_from_soft_jeita(chg);
