...
//add for C1N/B2N sw settings }
	...
	rc = of_property_read_u32(node, "fih,sdp-current-ua",
				&chg->sdp_current_ua);
	if (rc < 0)
		chg->sdp_current_ua = -EINVAL;

	rc = of_property_read_u32(node, "fih,hvdcp-current-ua",
				&chg->hvdcp_current_ua);
	if (rc < 0)
		chg->hvdcp_current_ua = -EINVAL;

	chg->aicl_rerun_en = of_property_read_bool(node, "fih,aicl-rerun");
	chg->jeita_recover_en = of_property_read_bool(node, "fih,jeita-recover");
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG,
			...
			chg->aicl_rerun_en ? USBIN_AICL_RERUN_EN_BIT : 0);
	if (rc < 0) {
