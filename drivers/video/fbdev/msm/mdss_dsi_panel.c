		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			{
				...
				if (bl_level == 0)
				{
					...
					#else
					pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOOO mdss_aod_resume_config(pdata)\n", __func__);
					#endif
					//SW4-HL-Display-GlanceMode-00+}_20170524
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			{
				...
				if (ret)
				{
					...
					//***********************************
					pr_err("[HL]%s: START of Power Down TP IC and DDIC\n", __func__);
					...
					gpio_free(ctrl->rst_gpio);
					pr_err("[HL]%s: Pull LOW DDIC RESET pin\n", __func__);
					...
					pr_err("[HL]%s: Pull LOW DDIC/TPIC POWER\n", __func__);
					...
					gpio_free(ctrl->tp_rst_gpio);
					pr_err("[HL]%s: Pull LOW TP RESET Pin\n", __func__);
					pr_err("[HL]%s: END of Power Down TP IC and DDIC\n", __func__);
					//***********************************
					...
					//***********************************
					pr_err("[HL]%s: START of Power UP TP IC and DDIC\n", __func__);
					pr_err("[HL]%s: Pull HIGH DDIC/TPIC POWER\n", __func__);
					ret = msm_dss_enable_vreg(
							ctrl->panel_power_data.vreg_config,
					...
					gpio_free(ctrl->tp_rst_gpio);
					pr_err("[HL]%s: Pull HIGH TP RESET Pin\n", __func__);
					...
					if (ret)
						pr_err("%s: Panel reset failed. rc=%d\n",
								__func__, ret);
					gpio_free(ctrl->rst_gpio);
					pr_err("[HL]%s: Pull HIGH DDIC RESET pin\n", __func__);
					pr_err("[HL]%s: END of Power UP TP IC and DDIC\n", __func__);
					//***********************************
	...
	if (on_cmds->cmd_cnt)
	{
		...
		if ((strstr(saved_command_line, "androidboot.device=C1N") != NULL)||(strstr(saved_command_line, "androidboot.device=PL2") != NULL)
	...
	int enable)
{
	...
	#else
	pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOOO fih_mdss_lp_config(pdata,enable,ctrl->ndx)\n", __func__);
	#endif
	//SW4-HL-Display-GlanceMode-00+}_20170524
