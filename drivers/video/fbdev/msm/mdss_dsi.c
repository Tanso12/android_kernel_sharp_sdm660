		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			{
				...
				{
					pr_debug("[HL]%s: gdouble_tap_enable is FALSE\n", __func__);
				...
				{
					pr_debug("[HL]%s: gdouble_tap_enable is TRUE\n", __func__);

					//No Nothing
					pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOO Pull LOW LCM Reset Pin\n", __func__);
				}
			}
			break;
	...
	switch (ctrl_pdata->panel_data.panel_info.panel_id)
	{
		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			...
			else
			{
				pr_debug("[HL]%s: gdouble_tap_enable is TRUE\n", __func__);
				//Do Nothing
				pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOO Cuf off LCM Powers\n", __func__);
			}
			break;
		//SW4-HL-Touch-ImplementDoubleTap-00+}_20170623
		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			{
				...
				else
				{
					...
					//No Nothing
					pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOO Pull LOW TP Reset Pin\n", __func__);
				}
			}
			break;
		default:
		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			{
				...
				else
				{
					...
					else
					{
						...
						//No Nothing
						pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOO Give LCM Powers\n", __func__);
					}
				}
			}
			break;
		...
		case FIH_ILI7807E_1080P_VIDEO_PANEL:
			{
				...
				else
				{
					...
					//Do Nothing
					pr_debug("[HL]%s: NOOOOOOOOOOOOOOOOOO Pull HIGH TP Reset Pin\n", __func__);
				}
			}
			break;
		default:
...
static int mdss_dsi_unblank(struct mdss_panel_data *pdata)
{
	...
	if (ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_LP) {
		...
		{
			pr_debug("[HL]%s: ctrl_pdata->low_power_config(pdata, false) <-- START\n", __func__);
			ret = ctrl_pdata->low_power_config(pdata, false);
			pr_debug("[HL]%s: ctrl_pdata->low_power_config(pdata, false) <-- END\n", __func__);
		}
		if (!ret)
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_PANEL_OFF:
		...
		switch (ctrl_pdata->panel_data.panel_info.panel_id)
		{
			...
			case FIH_FT8716_FFD_VIDEO_PANEL:
				{
					pr_err("\n\n*** [HL] %s: FIH_FT8716_FFD_VIDEO_PANEL ***\n\n", __func__);
