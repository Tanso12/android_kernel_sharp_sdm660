...
//for inter ker call
int mv4d_projector_i2c_write_ker(uint8_t cmd, uint8_t* data)
{
    int ret;
    uint8_t i2c_buf[2] = {0};
    struct i2c_msg msg[] = {
   ...
   {
    pr_err("%s ,ERROR g_client->addr is null ", __func__);
    return -ENXIO;
   }
    i2c_buf[0] = cmd;               // command
    i2c_buf[1] = data[0];

    dev_info(&g_client->dev, "%s cmd:0x%02X.gclent=%p\n", __func__, cmd,g_client);
        dev_info(&g_client->dev, "%s cmd:0x%02X. i2cbuf_1=0x%x\n", __func__, cmd,i2c_buf[1]);
    ret = i2c_transfer(g_client->adapter, msg, ARRAY_SIZE(msg));
    if (ret < 0) {
        dev_err(&g_client->dev, "%s: transfer failed.", __func__);
        return ret;
    } else if (ret != ARRAY_SIZE(msg)) {
        dev_err(&g_client->dev, "%s: transfer failed(size error).",
                __func__);
        return -ENXIO;
    }
//  i2c_smbus_write_byte_data(g_client,i2c_buf[0],i2c_buf[1]);
    return 0;
}

int mv4d_projector_i2c_read_ker(uint8_t cmd, uint8_t* data)
{
    int ret;
    uint8_t i2c_buf[2];
    uint8_t rx_buf[1];
   ...
   else
   {
    ...
    dev_info(&g_client->dev, "%s cmd:0x%02X.\n", __func__, cmd);

    ret = i2c_transfer(g_client->adapter, msgs, ARRAY_SIZE(msgs));
    if (ret < 0)
    {
        dev_err(&g_client->dev, "%s: transfer failed.\n", __func__);
        return ret;
    }
    else if (ret != ARRAY_SIZE(msgs)) {
        dev_err(&g_client->dev, "%s: transfer failed(size error).\n",
    ...
    data[0] = rx_buf[0];
    dev_info(&g_client->dev, "%s read cmd:0x%02X.i2c_buf0=0x%x,\n", __func__, cmd,data[0]);
